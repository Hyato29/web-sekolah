    ...
    <div x-data="{ openModal: false }">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500">Kelola akun akses dan profil tenaga pengajar aktif.</p>
            </div>
            <button @click="openModal = true" class="bg-[#1E3A8A] hover:bg-blue-800 text-white px-4 py-2 rounded-lg font-medium transition flex items-center gap-2">
                <i class='bx bx-user-plus text-xl'></i> Tambah Guru
            </button>
        </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="text-gray-700 text-sm">
                        ...
                        <tr class="border-b border-gray-50 hover:bg-blue-50/50 transition">
                            <td class="py-4 px-6 text-gray-500">{{ $guru->firstItem() + $index }}</td>
                            <td class="py-4 px-6 font-medium font-mono text-gray-600">{{ $item->nip ?? '-' }}</td>
                            <td class="py-4 px-6 font-bold text-gray-900">{{ $item->user->name }}</td>
                            <td class="py-4 px-6"><span class="px-3 py-1 bg-blue-100 text-[#1E3A8A] rounded-full text-xs font-semibold">{{ $item->mapel->nama_mapel ?? '-' }}</span></td>
                            <td class="py-4 px-6">{{ $item->user->email }}</td>
                            <td class="py-4 px-6 text-center space-x-2 flex justify-center">
                                <form action="{{ route('admin.guru.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus guru ini beserta akses loginnya secara permanen?');">
                                    @csrf @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="6" class="py-8 px-6 text-center text-gray-500">Belum ada data guru.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
                ...
                <div x-show="openModal" @click.away="openModal = false" class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all sm:w-full sm:max-w-2xl">
                    <form action="{{ route('admin.guru.store') }}" method="POST">
                        ...
                        <div class="bg-white px-6 py-5 border-b border-gray-100 flex justify-between items-center">
                            <h3 class="text-xl font-bold text-gray-900">Pendaftaran Akun Guru Baru</h3>
                        ...
                        <div class="px-6 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="col-span-1 space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="email" name="email" required class="w-full rounded-lg border-gray-300 py-2 px-3 border focus:ring-[#1E3A8A] focus:border-[#1E3A8A] text-sm" placeholder="guru@sekolah.com">
                                </div>
                                <div>
                            ...
                            <div class="col-span-1 space-y-4 md:border-l md:border-gray-100 md:pl-4">
                                <h4 class="text-sm font-semibold text-[#1E3A8A] uppercase tracking-wider mb-2">Profil Pegawai</h4>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">NIP (Opsional)</label>
                                    <input type="text" name="nip" class="w-full rounded-lg border-gray-300 py-2 px-3 border focus:ring-[#1E3A8A] text-sm" placeholder="Nomor Induk Pegawai">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Lengkap & Gelar</label>
                                    <input type="text" name="nama" required class="w-full rounded-lg border-gray-300 py-2 px-3 border focus:ring-[#1E3A8A] text-sm" placeholder="Cth: Budi Santoso, S.Pd">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Mata Pelajaran yang Diampu</label>
                                    <select name="mapel_id" required class="w-full rounded-lg border-gray-300 py-2 px-3 border focus:ring-[#1E3A8A] text-sm">
